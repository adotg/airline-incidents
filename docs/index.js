    ...
    resp.json().then(data => {
        ...
        const canvas2 = muze()
            ...
            .config({
                ...
                interaction: {
                    tooltip: { formatter: dataModel => tooltipFormatter(dataModel, ['Date', "Number of Incidents"]) }
                }
            })
        ...
        airlines.forEach((e, i) => {
            ...
            const canvas = muze()
                ...
                .config({
                    axes: {
                        x: { tickValues: dates, domain: [dates[0], dates[dates.length - 1]] },
                        y: { name: "Number of Incidents", domain: [0, 60] }
                    },
                    border: {
                        showValueBorders: { left: false, bottom: false }
                    },
                    interaction: {
                        tooltip: { formatter: dataModel => tooltipFormatter(dataModel, ['Date', "Number of Incidents by Airline"]) }
                    }
                    ...
                    .layers([
                        { mark: 'line' },
                        {
                            ...
                            encoding: {
                                ...
                                text: {
                                    field: 'Date',
                                    formatter: (value) => { return `Introduction of AirBus A320 Neo`; }
                                },
                                x: 'Date',
                                color: { value: () => '#414141' },
                            },
